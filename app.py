...
# --- Lightweight deterministic parser for "I want …" (handles landcover classes) ---
def try_parse_fetch_from_text(user_msg: str):
    ...
    and also single landcover requests like 'I want croplands'.
    """
    ...
    for t in tokens:
        # normalize "bioX" like bio1..bio19
        ...
        # landcover subclasses (e.g., croplands, closed_shrublands, etc.)
        if t in LANDCOVER_CLASSES and t not in classes:
            classes.append(t)
            continue

    # If user asked for subclasses, make sure 'landcover' is included
...
def chat_step(file, user_msg, history, state):
    # --- Quick intent: "what/which layers are available/supported?" ---
    ...
    # --- NEW: try deterministic fetch intent first (no LLM) ---
...
# --- Upload callback (shows layers list once) ---
def on_upload(f, history, state):
    ...
    if f and hasattr(f, "name"):
        ...
        if lat and lon:
            ...
            # ask to pick columns/CRS (we'll show layers list after CRS confirm to avoid duplication)
...
# --- CRS confirm callback (shows layers list if not yet shown) ---
...
with gr.Blocks() as demo:
    ...
    gr.Markdown("""
                ...
                """)
    # include our 'layers_help_shown' flag to avoid duplicates
